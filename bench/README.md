...
for config C (real, permissive-license, popular):
    ...
    for task T in 4 deterministic tasks:
        ...
        metrics: tokens_in · tokens_out · wall_clock · turns · pass/fail
...
- 4 tasks × N repeats is a pilot scale, not science; report medians and say so.
...
- Token accounting: session logs carry turns/bytes; provider-reported in/out tokens are taken from `--mode=json` when available (runner hook).
